...
/* ... keep your MOCK_* arrays and helper components (KPICard, SectionTitle, TypingIndicator, MessageRow, ChatComposer) exactly the same ... */
/* For brevity I will reuse them as in your original file (they're unchanged) */
...
/* Put your helper components KPICard, SectionTitle, TypingIndicator, MessageRow, ChatComposer here unchanged
   (I omitted replicating them here to keep this snippet concise — copy them from your working file).
   The important parts follow: the new showCongrats state/effect and the rendering of WelcomeCongrats.
*/
...
export default function ConseqXCEODashboard() {
  ...
  // NEW: control showing congrats splash when navigated from upgrade
  const [showCongrats, setShowCongrats] = useState(false);
  ...
  // place this useEffect with other top-level useEffects (runs once on mount)
  useEffect(() => {
    ...
    // check for the flag set by UpsellModal and show congrats if present
    try {
      const next = localStorage.getItem("show_congrats_next");
      if (next === "true") {
        // clear immediately to avoid repeat on refresh
        localStorage.removeItem("show_congrats_next");
        setShowCongrats(true);
        // hide after duration (1.5s)
...
Would you like me to create an action plan?`;
  ...
  return (
    <div className={`${darkMode ? "bg-gradient-to-b from-gray-900 to-gray-800 text-gray-100" : "bg-gradient-to-b from-gray-50 to-gray-100 text-gray-900"} min-h-screen transition-colors duration-300`}>
      ...
      {/* If showCongrats is true render WelcomeCongrats overlay at top-level so user sees it on the page */}
      {showCongrats && (
        <div className="fixed inset-0 z-60 flex items-start justify-center pointer-events-none">
          <div className="mt-20 pointer-events-auto">
            <WelcomeCongrats open={true} onDone={() => setShowCongrats(false)} name={auth?.user?.name ? auth.user.name.split(" ")[0] : ""} durationMs={1500} />
          </div>
        </div>
      )}
              ...
              <div className="mt-4">
                {tab === "dashboard" ? (
                  ...
                  <section>
                    <div className="mb-3 flex items-center justify-end">
                      <div className="flex items-center gap-2">
                        <button id="explorer-toggle" className="px-3 py-2 rounded-md border md:hidden" onClick={(e) => { e.stopPropagation(); setShowConversationListMobile((s) => !s); }} aria-expanded={showConversationListMobile}>Explorer</button>
                      </div>
                    </div>
