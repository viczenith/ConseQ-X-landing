...
const SystemOfInvestigation = ({
  ...
  const handleAnswer = (assessmentId, questionIndex, answer) => {
    ...
    if (currentQuestion < currentCard.questions.length - 1) {
      ...
      const updated = new Set(completedAssessments);
  ...
  const getProgress = id =>
    answers[id] ? Object.keys(answers[id]).length : 0;
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {system.subAssessments.map(assessment => {
                const done = completedAssessments.has(assessment.id);
              ...
              <div className="space-y-4">
                {["Yes", "Not Sure", "No"].map(option => (
                  <motion.button
                    ...
                    className={`p-4 rounded-xl border-2 text-center font-medium w-full flex items-center justify-center transition-colors ${
...
export default SystemOfInvestigation;
