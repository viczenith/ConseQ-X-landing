...
//     const assessment = system.subAssessments.find(a => a.id === assessmentId);
//     const maxScore = assessment.questions.length * 10;
    
//     switch(assessmentId) {
//       case 'crb':
//         if (score >= 140 && score <= maxScore) return '✅ Highly Constructive CRBs';
//         if (score >= 90 && score < 140) return '🟡 Moderate CRBs';
//         return '🔴 Detrimental CRBs';
      
//       case 'internal-client':
//         if (score >= 41 && score <= maxScore) return '✅ Excellent Service Culture';
//         if (score >= 31 && score < 41) return '🟡 Good But Uneven';
//         if (score >= 21 && score < 31) return '⚠️ Needs Redesign';
//         return '🔴 Service Dysfunction';
      
//       case 'dependency-mapping':
//         if (score >= 41 && score <= maxScore) return '✅ Healthy & Resilient';
//         if (score >= 31 && score < 41) return '🟡 Stable but Needs Attention';
//         if (score >= 21 && score < 31) return '⚠️ Vulnerable or Misaligned';
//         return '🔴 High-Risk Dependency';
      
//       case 'trust-flow':
//         if (score >= 41 && score <= maxScore) return '✅ High Trust & Flow';
//         if (score >= 31 && score < 41) return '🟡 Acceptable';
//         if (score >= 21 && score < 31) return '⚠️ Moderate Risk';
//         return '🔴 Critical Breakdown';
      
//       case 'silo-impact':
//         if (score >= 21 && score <= maxScore) return '✅ No Silo Present';
//         if (score >= 16 && score < 21) return '🟡 Manageable Risk';
//         if (score >= 11 && score < 16) return '⚠️ High Silo Risk';
//         return '🔴 Severe Silo Zone';
      
//       case 'breakdown-risk':
//         if (score >= 21 && score <= maxScore) return '✅ Strong Interaction';
//         if (score >= 16 && score < 21) return '🟡 Needs Tuning';
//         if (score >= 11 && score < 16) return '⚠️ Warning Zone';
...
//                   whileHover={{ scale: 1.05 }}
//                   whileTap={{ scale: 0.95 }}
//                   className={`px-6 py-3 rounded-lg font-medium flex items-center ml-auto ${
...
//                 >
//                   {currentQuestion === currentCard.questions.length - 1 
//                     ? 'Complete Assessment' 
//                     : 'Next Question'}
...
const InterdependencySystem = ({ 
  ...
  const handleAnswer = (assessmentId, questionIndex, answer) => {
    ...
    // Auto-swipe to next question after answering
    if (currentQuestion < currentCard.questions.length - 1) {
      setTimeout(() => {
        setCurrentQuestion(currentQuestion + 1);
      }, 300);
  ...
  // Calculate progress for each sub assessment
