...
//     // Auto-advance to next question or complete assessment
//     if (currentQuestion < currentCard.questions.length - 1) {
//       setTimeout(() => {
//         setCurrentQuestion(currentQuestion + 1);
//       }, 300);
//     } else {
//       // Mark assessment as completed and return to overview
...
//   const allCompleted = completedAssessments.size === system.subAssessments.length;
  
//   const getProgress = (assessmentId) => {
//     if (!answers[assessmentId]) return 0;
//     return Object.keys(answers[assessmentId]).length;
//   };
...
//               }`}>
//                 <h2 className={`font-bold mb-2 ${darkMode ? "text-yellow-400" : "text-yellow-600"}`}>
//                   Assessment Goal
//                 </h2>
//                 <p>{system.goal}</p>
//               </div>
...
//                   }`}>
//                     Progress: {completedAssessments.size} of {system.subAssessments.length} completed
//                   </h3>
//                   {allCompleted && (
...
//                     style={{ width: `${(completedAssessments.size / system.subAssessments.length) * 100}%` }}
//                   />
//                 </div>
//               </div>
//             </motion.div>
...
//                     } ${isCompleted ? (darkMode ? "border-green-500/50" : "border-green-500/30 bg-green-50/50") : ""}`}
//                     onClick={() => {
...
//                 }`}>
//                   <span className={`font-bold ${darkMode ? "text-indigo-300" : "text-indigo-800"}`}>🔗</span>
//                 </div>
//                 <div>
...
const InterdependencySystem = ({ 
  ...
  const handleSystemSelect = (assessment) => {
    setCurrentCard(assessment);
    setCurrentQuestion(0);
  };
        ...
        {!currentCard ? (
          <>
            <div className="flex items-center mb-8">
              <button 
                onClick={onBack}
                className={`flex items-center ${darkMode ? "text-indigo-400 hover:text-indigo-300" : "text-indigo-600 hover:text-indigo-800"}`}
              >
                <FaArrowLeft className="mr-2" /> Back to Systems
              </button>
            </div>
            ...
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              className={`rounded-2xl shadow-xl p-6 md:p-8 mb-8 ${darkMode 
                  ? "bg-gray-800/50 border border-gray-700" 
                  : "bg-white border border-gray-200"}`}
            >
              ...
              <div className={`mt-6 p-4 rounded-lg ${darkMode 
                  ...
                  : "bg-gray-100 border border-gray-200"}`}>
                <div className="flex justify-between items-center mb-2">
                  <h3 className={`font-semibold ${darkMode ? "text-green-400" : "text-green-600"}`}>
                    Progress: {completedAssessments.size} of {system.subAssessments.length} completed
                  </h3>
                  {allCompleted && (
                    <div className="flex items-center text-green-500">
              ...
              {system.subAssessments.map((assessment) => {
                ...
                return (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    whileHover={{ y: -5 }}
                    className={`p-6 rounded-xl border transition-all h-full flex flex-col cursor-pointer ${darkMode 
                          ? "bg-gray-800/30 border-gray-700 hover:border-yellow-500" 
                          : "bg-white border border-gray-200 hover:border-yellow-500"
                      } shadow-lg hover:shadow-xl ${isCompleted 
                        ? darkMode 
                          ? "border-green-500/50" 
                          : "border-green-500/30 bg-green-50/50"
                        : ""}`}
                    onClick={() => handleSystemSelect(assessment)}
                  >
                    ...
                    <div className="mt-auto">
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <span className="text-xs font-medium text-gray-500">
                          {isCompleted ? `${progress}/${totalQuestions}` : `${progress}/${totalQuestions}`}
                        </span>
                      </div>
                      <div className="w-full bg-gray-200 rounded-full h-2">
                        <div 
                          className={`h-2 rounded-full ${isCompleted ? 'bg-green-500' : progress > 0 ? 'bg-yellow-500' : 'bg-gray-300'}`}
                          style={{ width: `${progressPercent}%` }}
                        />
                      </div>
                    </div>
          ...
          >
            <div className="p-6 border-b border-gray-200">
              <button 
                onClick={() => setCurrentCard(null)}
                className={`flex items-center mb-4 ${darkMode ? "text-indigo-400 hover:text-indigo-300" : "text-indigo-600 hover:text-indigo-800"}`}
              >
                <FaArrowLeft className="mr-2" /> Back to Assessments
              </button>
              
              <div className="flex items-start">
                <div className={`p-3 rounded-lg mr-4 ${darkMode ? "bg-indigo-900/30" : "bg-indigo-100"}`}>
                  <span className={`font-bold ${darkMode ? "text-indigo-300" : "text-indigo-800"}`}>
                    🔗
                  </span>
                </div>
                <div>
            ...
            <div className="p-6">
              <div className="flex justify-between items-center mb-6">
                ...
                <div className="w-32 bg-gray-200 rounded-full h-2">
                  <div 
                    className="h-2 bg-gradient-to-r from-blue-500 to-indigo-600 rounded-full"
                    style={{ width: `${((currentQuestion + 1) / currentCard.questions.length) * 100}%` }}
                  />
                </div>
              </div>

              <div className="mb-8">
                <h4 className={`text-xl font-medium mb-4 ${darkMode ? "text-gray-200" : "text-gray-800"}`}>
                  {currentCard.questions[currentQuestion].statement}
                </h4>
              ...
              <div className="space-y-4">
                {['Yes', 'Not Sure', 'No'].map((option) => (
                  <motion.button
                    ...
                    whileHover={{ scale: 1.03 }}
                    whileTap={{ scale: 0.98 }}
                    className={`p-4 rounded-xl border-2 text-center font-medium transition-colors w-full flex items-center justify-center ${answers[currentCard.id]?.[currentQuestion] === option
                        ? option === 'Yes' 
                          ? darkMode 
                            ? 'bg-green-900/20 border-green-600 text-green-300' 
                            : 'bg-green-100 border-green-500 text-green-700' 
