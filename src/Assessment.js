...
export default function AssessmentPlatform() {
  ...
  const generateAIAnalysis = (scores) => {
    ...
    if (completedSystems.length > 0) {
      overallScore = completedSystems.reduce((sum, id) => {
        const system = scores[id];
        return sum + (system.systemScore / system.maxSystemScore);
      }, 0) / completedSystems.length * 100;
    }
    ...
    Object.entries(scores).forEach(([systemId, systemScore]) => {
      ...
      const interpretation = systemScore.interpretation;
      analysis += `**Interpretation:** ${interpretation.rating} - ${interpretation.interpretation}\n\n`;
      ...
      system.subAssessments.forEach(sub => {
        ...
        const scoreText = `${subScore.score}/${subScore.maxScore}`;
        const interpretation = subScore.interpretation;
        const interpretationText = `${interpretation.rating}: ${interpretation.interpretation}`;
        
        analysis += `| ${sub.title} | ${scoreText} | ${interpretationText} |\n`;
      });
      analysis += "\n";
      ...
      if (interpretation.recommendations && interpretation.recommendations.length > 0) {
        interpretation.recommendations.forEach((rec, idx) => {
          analysis += `${idx + 1}. ${rec}\n`;
        });
      } else {
        analysis += "No specific recommendations available for this system.\n";
      }
      analysis += "\n";
    });
