...
import useFreemium from "./hooks/useFreemium";            // <- provided
import UpsellModal from "./components/UpsellModal";       // <- provided
import { useAuth } from "./contexts/AuthContext";         // <- provided
...
export default function AssessmentPlatform() {
  ...
  const auth = useAuth(); // mock auth/context; provided in contexts/AuthContext.js
  const freemium = useFreemium({ freemiumDurationHours: 24, maxRunsPerDay: 3 }); // 3 runs/day (hook kept for UI but per-org storage used)
  const navigate = useNavigate();

  // ======= Permission / freemium helpers =======
  const FREE_RUNS_KEY = "conseqx_free_runs_v1"; // mapping: orgId -> { expiresAt, count }
  const FREE_RUNS_LIMIT = 3;

  function isOrgPremium() {
    // AuthContext may expose org at auth.org or auth?.org; handle both shapes for compatibility
  ...
  // Get orgId for storage mapping; use user.orgId if present else 'anonymous'
  function getOrgIdForStorage() {
    const u = auth?.user || auth?.user; // safe access
    const org = auth?.org || (u && u.orgId ? null : null);
    // prefer org id from auth.org, then user.orgId, finally email hashed or 'anonymous'
    const orgId = (auth && auth.org && auth.org.id) || (u && u.orgId) || (u && u.id) || "anonymous";
    return orgId;
  }

  // Check if the org (or anonymous) has uses left
  ...
  // Increment the run count for the org; returns { remaining, count, expiresAt }
  function recordFreemiumRun() {
    ...
    state[orgId] = newCur;
    localStorage.setItem(FREE_RUNS_KEY, JSON.stringify(state));
    // Try to sync the freemium hook remainingRuns if available
    try {
      if (freemium && typeof freemium.consumeRun === "function") freemium.consumeRun();
    } catch (e) {
      // ignore
    }
  ...
  // helper to open freemium-expired modal and close others (so it takes precedence)
  function openFreemiumExpiredModal() {
    ...
    // finally show the freemium expired modal
  ...
  // computeFinancialImpact (kept shorter here — full previously provided logic can be pasted)
  const computeFinancialImpact = () => {
    // simplified: reuse your previous function in production
    setFinanceAnalysis("Financial Impact computed (mock)");
  };
  ...
  const generateAnalysis = async () => {
    ...
    try {
      const scoresObj = {};
      const completedSystems = systems.filter((sys) => sys.subAssessments.every((sub) => answers[sub.id] && Object.keys(answers[sub.id]).length === sub.questions.length));
      ...
      // freemium gating: check remaining uses BEFORE running LLM
      const can = canRunFreemiumAction();
      if (!can.allowed) {
        // block and show freemium expired modal (highest precedence)
        openFreemiumExpiredModal();
        setGeneratingAnalysis(false);
        return;
      }

      // record run (we increment before running the LLM so the counter properly reflects this run).
      const record = recordFreemiumRun();
      // record = { remaining, count, expiresAt }

      // call generateSystemReport (your util may call LLM; here we just call the provided util)
      ...
      setAnalysisContent(analysisText || "Mock analysis (no LLM configured)");

      // If this run exhausted the free runs, open the freemium expired modal AFTER the run completes (so the third run is allowed, then the modal appears).
      if (record.remaining <= 0) {
        // show modal and close other overlays
        openFreemiumExpiredModal();
      }
    } catch (error) {
  ...
  // Handlers that gate freemium/premium usage
  ...
  // Auth modal handlers (register/login) - using mock auth context
  ...
  async function handleLoginSubmitForCEO({ email, password, orgName, ceoName, phone }) {
    ...
    try {
      ...
      console.error(err);
      // fallback to register in permissive mock
      try {
        const realOrg = orgName || userInfo.organization || authForm.orgName || "Org";
  ...
  // CEOPromptModal - improved local state for inputs so typing won't be interrupted by parent re-renders.
  function CEOPromptModal() {
    ...
    // local state ensures stable controlled inputs (prevents focus-loss typing bug)
            ...
            <div className={`${sectionBg} p-4 rounded-lg border ${cardBorder}`}>
              ...
              {!localIsLogin ? (
                <form onSubmit={submitRegister} className="space-y-3" autoComplete="off" spellCheck={false}>
                  ...
                  <div className="flex items-center justify-between">
                    <button type="submit" disabled={localProcessing} className={primaryBtnClass}>
                      {localProcessing ? "Registering..." : "Create CEO Dashboard"}
                    </button>
                ...
                <form onSubmit={submitLogin} className="space-y-3" autoComplete="off" spellCheck={false}>
                  ...
                  <div className="flex items-center justify-between">
                    <button type="submit" disabled={localProcessing} className={primaryBtnClass}>
                      {localProcessing ? "Signing in..." : "Sign in & Open Dashboard"}
                    </button>
                    <button type="button" onClick={() => setShowCEOPrompt(false)} className={smallBtnClass}>Cancel</button>
                  </div>
                </form>
  ...
  //
  // FreemiumExpiredModal - dark-mode aware and clean (ChatGPT-like)
  //
  function FreemiumExpiredModal() {
    ...
    function openUpgrade() {
      // close freemium expired and open upsell
      setShowFreemiumExpired(false);
      setShowUpsellModal(true);
    }
          ...
          <div className={`p-6 ${darkMode ? "bg-gradient-to-r from-gray-900/60 to-gray-800/60 text-white" : "bg-gradient-to-r from-white to-gray-50 text-gray-900"}`}>
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className={`mt-2 text-sm ${textSubtle}`}>Your free Conseq-X analyses limit has been reached for this window. Upgrade to Conseq-X Ultra for unlimited analyses, CEO dashboard and priority support.</p>
              </div>
              <button onClick={closeAndDismiss} className="text-gray-400 hover:text-gray-600" aria-label="Close">
                <FaTimes />
              </button>
            </div>
          </div>
          ...
          <div className={`p-6 ${cardBg}`}>
            <ul className={`mb-4 space-y-2 text-sm ${textSubtle}`}>
              <li className="flex items-start gap-3">
                ...
                <div><strong className={`${darkMode ? "text-white" : "text-gray-900"}`}>Unlimited AI analyses</strong> — run as many reports as your team needs.</div>
              </li>
              <li className="flex items-start gap-3">
                <div className="mt-0.5 text-blue-500"><FaCheck /></div>
  ...
  return (
    <div className={`min-h-screen font-sans overflow-x-hidden transition-colors duration-500 ${darkMode ? "bg-gradient-to-b from-gray-900 to-gray-800 text-gray-200" : "bg-gradient-to-b from-gray-50 to-gray-100 text-gray-800"}`}>
      ...
      <UpsellModal open={showUpsellModal} onClose={() => setShowUpsellModal(false)} onUpgrade={() => { auth.upgrade && auth.upgrade(); setShowUpsellModal(false); }} />
      ...
      {/* Results / Booking / Thankyou / Finance modals (kept your original content) */}
      <AnimatePresence>
        {activeModal && (
                ...
                {activeModal === "results" && (
                  <div className="space-y-5">
                    <div className="text-center">
                      ...
                      <p className={`${darkMode ? "text-blue-300" : "text-blue-600"}`}>{analysisContent ? "Your AI Analysis Report" : "Select your report preferences"}</p>
                    </div>
                    ...
                    {generatingAnalysis ? (
                      <div className="text-center py-8">
                        <div className="flex flex-col items-center justify-center">
                          ...
                          <p className={`text-lg ${darkMode ? "text-gray-300" : "text-gray-700"}`}>Conseq-X Analysing... <br /><span className="text-yellow-500 font-semibold"><i>Please wait for your result</i></span></p>
                        </div>
                      </div>
                    ) : analysisContent ? (
                      <div className="space-y-5">
                        <div ref={analysisRef} className={`p-6 rounded-xl ${darkMode ? "bg-gray-800/30 border border-gray-700" : "bg-white border border-gray-200"}`}>
                        ...
                        <div className="flex flex-wrap justify-center gap-4">
                          <button onClick={() => setAnalysisContent(null)} className="px-4 py-2 bg-gray-200 text-gray-800 rounded-lg">Back to Options</button>
                          <button onClick={handleDownloadPDF} className="px-4 py-2 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-lg flex items-center"><FaDownload className="mr-2" /> Download PDF Report</button>
                          <button onClick={handleScheduleClick} className="px-4 py-2 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-lg">Schedule Consultation</button>
                          <button onClick={handleOpenFinance} className="px-4 py-2 bg-gradient-to-r from-purple-500 to-indigo-500 text-white rounded-lg">Financial Impact Analysis</button>
                        </div>
                      </div>
                    ) : (
                      <>
                ...
                {activeModal === "thankyou" && (
                  <div className="text-center space-y-5 p-8">
                    ...
                    <div className="flex flex-col items-center mt-6">
                      ...
                      <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => { setActiveModal(null); setStep(0); setAnswers({}); setAnalysisContent(null); }} className="px-4 py-2 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-lg">Complete Assessment</motion.button>
                    </div>
                  </div>
                )}
              </div>
